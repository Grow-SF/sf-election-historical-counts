...
test("county night-share timeline renders its title, one panel per shown county, and a dimmed primary line alongside the solid general line", () => {
  ...
  // this chart shows exactly one control panel (San Francisco, the
  // no-new-tech baseline drawn in every other panel too) plus one panel per
  // complete TREATED county -- the other control counties (Del Norte, Lake,
  // Mendocino, Tehama, Colusa) don't get their own panel here regardless of
  // how much of their own data is sourced; that's CountyNightShareChart's
  // per-control pre/post bar, a different chart. Unaffected by either change
  // in this commit.
  for (const name of [
    "San Francisco",
    "Los Angeles",
