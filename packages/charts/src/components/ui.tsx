 ...
 * "who could vote" charts. Use as a chart child: {eventLines(1962, 2028, gold, faint)}
 *
 * `gold`/`faint` are passed in from the calling component (which holds the
 * theme) because this is a plain helper, not a component, so it can't call the
 * theme hook itself.
 */
...
export function Section({
  ...
  return (
    <section
      id={id}
      className="mx-auto max-w-5xl scroll-mt-16 px-5 py-16 sm:py-20"
    >
...
export function DualRange({
  ...
  return (
    <div className={`rangepair ${className ?? ""}`}>
      ...
      <div
        className="fill"
        style={{ left: at(lo), right: `${100 - ((hi - min) / span) * 100}%` }}
      />
      <input
        type="range"
        min={min}
        ...
        value={lo}
        onChange={(e) =>
          onChange(Math.min(Number(e.target.value), hi - minGap), hi)
        }
      ...
      <input
        ...
        value={hi}
        onChange={(e) =>
          onChange(lo, Math.max(Number(e.target.value), lo + minGap))
        }
