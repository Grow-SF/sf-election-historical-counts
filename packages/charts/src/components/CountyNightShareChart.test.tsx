...
test("county night-share chart renders its title, all four type toggles, and the presidential-general controls", () => {
  ...
  // default view is presidential generals. San Francisco, Colusa, Del Norte,
  // Lake, Mendocino, and Tehama are the no-new-tech controls that clear the
  // pre/post bar here: each has its own earliest and most recent sourced
  // presidential-general point (the control pre-window is anchored to each
  // county's own data, not a shared adoption-year cutoff -- see prepost()).
  ...
  expect(screen.getAllByText("no new tech")).toHaveLength(6);
