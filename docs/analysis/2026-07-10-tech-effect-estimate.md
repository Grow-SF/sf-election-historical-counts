...
The conclusion is unavoidable and must be stated first, not buried: **at this panel the measured e-pollbook effect is smaller than the effect the same estimator produces from counties that were never treated at all.** We cannot yet distinguish the e-pollbook effect from the noise floor set by the control group's composition: San Francisco plus a handful of small rural never-adopter counties, where the placebo noise comes from the small-county side (the fake effects above are Mendocino's and Del Norte's, with SF among the stable controls they are measured against). Every "effect" in this document is written conditionally for that reason.
