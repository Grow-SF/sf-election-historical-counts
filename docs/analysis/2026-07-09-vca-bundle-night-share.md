...
## Post-merge recomputation (2026-07-10): the corrected panel moves the RI verdict from Pattern A to Pattern B

The origin/main merge landed the Secretary of State status-page sweep (87 applied cells, three corrections to previously committed values, primaries as a second election type) plus five new rural control counties and the Nevada tech re-adjudication. This section reruns the estimator, unchanged in design, against the merged panel (90 usable general-election rows in the core 16-jurisdiction universe; the control set stays C = {SF, SLO} for continuity; primaries are excluded by the generals-only filter). All numbers are emitted by the seeded script and byte-identical across reruns.

**Headline (theta_2024, type-matched, n = 13):** +10.01 pp pooled (vs SF +7.16, vs SLO +12.87); RI p = 0.1163 (MC SE 0.0032, null SD 6.02); 90% RI CI [0.0, 19.5]; MDE 14.99 pp (alpha 0.10, power 0.8). Placer and San Bernardino join the 2024 aggregate for the first time (the SoS sweep filled their missing generals), and Riverside 2024 rejoins as comparable at its corrected 57.11. Secondary family (Holm): theta_2022 +7.67 (n = 12), holm_p 0.875; joint pre-trend RI p = 0.9209 (clean); no downgrade triggers fired.

**What changed and why.** On the pre-merge panel this document reported theta_2024 = +10.07, RI p = 0.0663, Pattern A. The point estimate is essentially unchanged (+10.01), but the RI p roughly doubled to 0.1163, crossing the 0.10 line, so the selection rule now picks Pattern B. The movement is not driven by the estimate weakening; it is driven by the null distribution widening: the corrected values (Riverside 2016 to 62.57 from 65.94, Riverside 2014 to 74.29, Napa 2014 to 50.34) and the two newly identified treated counties change which placebo reassignments are valid and how much spread they produce (null SD 6.02 vs 5.67 pre-merge; theta_2024 redraw rate fell from 0.77 to 0.35 as more counties can serve as valid placebo controls).

**Three methods, one honest summary.** The main campaign's estimator (docs/analysis/2026-07-10-vca-bundle-tech-effect.md, post-integration section) reads +7.64 pp with a jackknife 95% CI of (0.84, 14.44), which excludes zero, while its randomization placebo finds 22 of 62 control-only splits (35.5 percent) at least as extreme as that headline. Our randomization-inference test on the same merged data (different estimator construction, different randomization scheme) lands at p = 0.1163: a result this size arises in roughly 1 in 9 random adoption reassignments. Point estimates from all three constructions agree at roughly +8 to +10 pp in 2024. The inference methods split exactly along their assumptions: the jackknife, which treats counties as independent units, certifies; both randomization approaches, which test the design itself, say not quite. That agreement-in-direction plus disagreement-at-the-certification-line is the same "one notch short" verdict the main campaign reached, now confirmed by an independently built estimator. The honest summary remains an interval, [0.0, 19.5], whose lower edge touches zero, not a verdict.
