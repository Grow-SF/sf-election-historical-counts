...
- No em dashes in any prose (notes, VERIFY.md, commit messages, CPRA drafts).
- Derived files (`packages/data/county_night.json`, `MACHINE_CHECK.md`, `HUMAN_VERIFY.md`) are regenerated, never edited.

## Row-Landing Procedure (referenced by Tasks 3-11; "land the row" means exactly this)
...
### Task 1: CPRA request drafts (fire first, latency is weeks)

**Files:**
- Create: `docs/research/records-requests/2026-07-09-county-en-cpra.md`

**Interfaces:**
- Produces: email drafts the operator sends; a tracking table other tasks consult before re-researching a row.

- [ ] **Step 1: Write the drafts file.** One section per request, each with: recipient (registrar public-records email, findable on the county ROV site), subject, body, and a tracking row (sent date, due date = +10 calendar days per Gov. Code 7922.535, response). Requests, most-likely-to-succeed first:
  1. **San Bernardino, Nov 8 2022 general:** the county's own archived Results Posting Schedule (sbcountyelections.com/Elections/2022/1108/ResultSchedule/, Wayback capture 20221118191058) shows a scheduled "Posting of Election Night" final report; request that report (the last cumulative results report generated on election night, Nov 8-9 2022, with its ballots-counted total).
  2. **Placer, Nov 5 2024 general:** request the "UNOFFICIAL RESULTS - REPORT N" series pasted to placercountyelections.gov/election-results/ on election night, specifically the last report generated before the canvass resumed.
  3. **Placer, Nov 8 2022 general:** same series for 2022.
  4. **Riverside, Nov 6 2018 general:** the last election-night cumulative report (the registrar's release schedule says hourly until ~3 a.m.).
  Body template (adapt per county): "Under the California Public Records Act I request the final cumulative election results report generated on election night for the <date> election, including its ballots-counted total and generation timestamp. This is the report posted before the official canvass resumed. Electronic copies by email are preferred."
- [ ] **Step 2: Commit.**
  ```bash
  git add docs/research/records-requests/2026-07-09-county-en-cpra.md
  git commit -m "docs: CPRA drafts for county election-night reports"
  ```
- [ ] **Step 3: Tell the operator** the file path and which 4 emails to send. Do not block on responses; when one arrives, land the row via the Row-Landing Procedure at `confidence: "primary"`.

### Task 2: NewsBank session bring-up and coverage probe
...
- Produces: a go/no-go per paper x year that Tasks 3-8 read before searching.
...
- [ ] **Step 3: Commit** the coverage file. Tasks 3-8 skip any cell recorded as a miss.

### Task 3: Sacramento 2016 (one row from a new chart panel)
...
**Interfaces:**
- Consumes: Task 2's coverage go/no-go; the live NewsBank session (serialize with Tasks 4-8).
...
### Task 4: Santa Clara 2016 and 2018 (Mercury News)
...
**Interfaces:**
- Consumes: Task 2 coverage; NewsBank session (after Task 3 finishes with it).
...
### Task 5: Madera 2012 and 2014 (Fresno Bee, Madera Tribune)
...
**Interfaces:**
- Consumes: Task 2 coverage; NewsBank session.
...
### Task 6: San Diego 2012, 2014, 2016 (Union-Tribune)
...
### Task 7: Fresno 2012, 2014, 2018, 2022 (Fresno Bee)
...
### Task 8: Riverside 2012-2018 and San Bernardino 2012-2022 (Press-Enterprise, SB Sun)
...
**Interfaces:**
- Consumes: Task 2 coverage; NewsBank session.
- Produces: dataset depth (these two need too many rows to reach the chart this pass, but every landed row feeds the pre/post comparison and the CPRA rows may complete them later).
...
### Task 9: Alternate web-archive sweep (rows still null after Tasks 3-8)
...
### Task 10: URL/ID enumeration probes and registrar social media
...
### Task 11: New-county scouting (Clarity CDN + press-release archives + control candidates)

**Files:**
- Create: `docs/research/night-recovery-2026-07-09/county-en/new-county-scout.md`

**Interfaces:**
- Produces: a ranked candidate memo with, per county: Clarity election IDs per year (or press-release archive URL), tech-adoption sketch, estimated rows recoverable. Tasks 12-13 consume it.
...
### Task 12: Add new county #1 end to end
...
**Interfaces:**
- Consumes: Task 11's top-ranked candidate.
...
### Task 13: Add control county end to end

Same file set and steps as Task 12, for Task 11's best no-new-tech control candidate. The `adoption` fields stay null and the county-tech records must document NON-adoption with sources (status "not-adopted" with evidence, not absence of evidence). The chart marks controls via `control: true` only for SF today; check `scripts/build_county_night.py` for how `control` is set, and if it is SF-hardcoded, extend the builder so any no-adoption county with a note gets `control: true`, with a vitest assertion in `packages/data/index.test.ts` that at least two control jurisdictions exist.
...
### Task 14: Rollup and operator packet
...
- [ ] **Step 3:** Write SUMMARY.md: panels before/after, rows landed (with confidence and verdict), rows still null and why, CPRA requests outstanding.
...
- Spec coverage: gap-filling (Tasks 3-10), CPRA early (Task 1), NewsBank probe risk (Task 2), new counties + control (Tasks 11-13), evidence hardening is folded into landing (verdicts, render_verified) rather than a separate pass, out-of-scope items untouched. Success criteria checked in Task 14.
- The chart itself changes only via regenerated `county_night.json`, except the possible `control` flag generalization in Task 13, which is data-plumbing, not presentation, and carries a test.
- Ordering: Task 1 first (latency), Task 2 gates 3-8 (serialized on the shared NewsBank session, highest-value county first), Tasks 9-11 are curl-only and can run parallel to nothing-else-using-the-browser, Tasks 12-13 depend on 11, Task 14 last.
