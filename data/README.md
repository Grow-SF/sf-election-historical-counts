...
**What:** the single-file rollup built to share with the SF Department of
Elections — one row per election (1849–present) with registration, total
turnout, election-night count, and the election-day / vote-by-mail split,
each value paired with a compacted source citation (full verbatim citations
live in the underlying files and `docs/sources.md`). Night counts flagged
`[partial or contest-sum floor]` are lower bounds; reused registrations are
flagged approximate. **Generated by** `scripts/build_consolidated_export.py`
— do not edit by hand. **Coverage:** 313 elections; 304 night counts,
242 registrations, 105 e-day/VBM splits.
